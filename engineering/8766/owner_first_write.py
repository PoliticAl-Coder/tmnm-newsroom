...
def safe_result(first,receipt,write_count,blocked,error="NONE"):
 ...
 return "\n".join(["TMNM 8766 OWNER FIRST WRITE","GUID="+GUID,"PAGE_ID="+PAGE_ID,"FACEBOOK_WRITE_COUNT="+str(write_count),"POST_ID="+str(first.post_id or "NONE"),"PUBLISH_STATE="+str(first.state),"DURABLE_RECEIPT="+durable,"SECOND_INVOCATION_BLOCKED="+("PASS" if blocked else "HOLD"),"ERROR="+error])+"\n"
def main():
