...
def collect(root: Path) -> list[tuple[Path, bytes]]:
    ...
    captured_paths={rel for rel,_ in captured}
    if not any(path in captured_paths for path in PUBLISH_SOURCE_PATHS):
        raise CollectorError("MISSING_EVIDENCED_PUBLISHING_SOURCE_PATH")
    if not any(path in captured_paths for path in OWNER_CONSOLE_SOURCE_PATHS):
        raise CollectorError("MISSING_EVIDENCED_OWNER_CONSOLE_SOURCE_PATH")
    return captured
